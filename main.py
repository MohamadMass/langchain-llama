...
If you don't know the answer, use your knowldge to come up with answer that is brief and straight to the point.
...
async def initialize_bot():
    ...
    welcome_message.content = (
        "Hi, I am SIA, your sorbet assistant, what can i help you with?"
    )
...
async def on_message(msg: cl.Message):
    if not msg.elements:
        await process_chat_message(msg)
        return

    # Check for PDF files
    pdf_files = [file for file in msg.elements if file.mime == 'application/pdf']
    if pdf_files:
        pdf_file = pdf_files[0]  # Assuming only one PDF is processed at a time
        pdf_content = pdf_file.content

        # Use a timestamp to create a unique filename
        timestamp = int(time.time())
        pdf_path = f'./data/pdf_file_{timestamp}.pdf'

        # Save the PDF to /data folder
        with open(pdf_path, 'wb') as f:
            f.write(pdf_content)

        # Run ingest.py script to process the PDF and update vector DB
        # Assuming ingest.py is designed to take the file path as an argument
        os.system(f'python ingest.py {pdf_path}')

        # Notify user of PDF processing
        print(pdf_file)
        # Now, the PDF content is part of the vector database and can be used in subsequent queries
    else:
        # Processing other files (e.g., images) as per your existing logic
        images = [file for file in msg.elements if "image" in file.mime]
        if images:
            image_bytes = images[0].content
            await cl.Message(content=f"Received {len(images)} image(s)").send()
        else:
            await cl.Message(content="File type not supported").send()

    # Continue with the regular chat message processing
    await process_chat_message(msg)
